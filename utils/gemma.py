...
def tensor_to_image(processor, device, original_image, pixel_values):
...
def iterative_FGSM(model, processor, dtype, device, image_rgb, target_label, epsilon=0.01, iterations=1, loss_threshold=0.0, progressive_save=False, out_path=None):
    ...
    for i in range(iterations):
        ...
        if (progressive_save):
            tensor_to_image(processor.image_processor, device, image_rgb, model_inputs['pixel_values']).save(out_path)
...
def main():
    ...
    # get target output
    adv_pixel_values = iterative_FGSM(
        model,
        processor,
        dtype,
        device,
    ...
    adv_img = tensor_to_image(processor.image_processor, device, image, adv_pixel_values)
